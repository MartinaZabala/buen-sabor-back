...
//	private ClienteRepository clienteRepository;
	...
	@Autowired
	private UniMedidaRepository unidadMedidaRepository;

	@Autowired
	private CategoriaRepository categoriaRepository;

	@Autowired
	private ArtInsumoRepository articuloInsumoRepository;

	@Autowired
	private ArtManufRepository articuloManufacturadoRepository;
	...
	CommandLineRunner init() {
		return args -> {
			...
			ArticuloManuf pizzaMuzarella = ArticuloManuf.builder().denominacion("Pizza Muzarella").descripcion("Una pizza clasica").unidadMedida(unidadMedidaPorciones).precioVenta(130.0).tiempoEstimadoMinutos(15).build();
			ArticuloManuf pizzaNapolitana = ArticuloManuf.builder().denominacion("Pizza Muzarella").descripcion("Una pizza clasica").unidadMedida(unidadMedidaPorciones).precioVenta(150.0).tiempoEstimadoMinutos(15).build();
			...
			Promocion promocionDiaEnamorados = Promocion.builder().denominacion("Dia de los Enamorados")
					...
					.descripciondescuento("Descuento rige solo este 14 de febrero ")
					.precioPromocion(500.00)
					.build();
